...
export default function SweeperAfterImageScreen({ navigation }: any) {
  ...
  const handlePhoto = async (base64: string) => {
    ...
    try {
      ...
      await api.post('/sweeping/after-image', {
        photo_base64: base64,
        gps_lat: coords?.latitude || 0,
        gps_lng: coords?.longitude || 0,
      });
      setMessage(t('sweeping.afterSubmitted'));
    } catch (e: any) {
  ...
  if (showCamera) {
    return <CameraCapture facing="back" requireFace={false} title={t('sweeping.afterInstruction')} onCapture={handlePhoto} onCancel={() => setShowCamera(false)} />;
  }

  return (
    <View style={styles.container}>
      <Header title={t('menu.afterImage')} showBack={true} onBack={() => navigation.goBack()} />
      <View style={styles.content}>
        <Text style={styles.instruction}>{t('sweeping.afterInstruction')}</Text>
        {message && <View style={styles.success}><Text style={styles.successText}>{message}</Text></View>}
        {error && <View style={styles.errorContainer}><Text style={styles.errorText}>{error}</Text></View>}
        {loading && <ActivityIndicator size="large" color={theme.colors.primary} />}
        <Button title={t('sweeping.captureAfter')} onPress={() => setShowCamera(true)} variant="primary" disabled={loading} />
      </View>
    </View>
  );
}
