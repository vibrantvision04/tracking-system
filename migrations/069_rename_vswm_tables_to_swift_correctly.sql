...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'route_types_vswm') THEN
        ALTER TABLE route_types_vswm RENAME TO route_types_swift;
    END IF;

    -- Rename the sequence if it exists
    IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'route_types_vswm_id_seq') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'vehicle_types_vswm') THEN
        ALTER TABLE vehicle_types_vswm RENAME TO vehicle_types_swift;
    END IF;

    -- Rename the sequence if it exists
    IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'vehicle_types_vswm_id_seq') THEN
