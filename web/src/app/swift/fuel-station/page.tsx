...
interface FuelStation {
  ...
  company_name: string;
  owner_name: string;
  owner_contact_1: string;
  owner_contact_2: string;
  address: string;
...
export default function FuelStationPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [stRes, coRes] = await Promise.all([
        ...
        api<{ data: FuelCompany[] }>("/api/fuel-companies"),
      ]);
  ...
  const closeForm = () => {
    ...
    setIsDrawing(false);
    setForm({ 
      name: "", company_id: "", owner_name: "", owner_contact_1: "", owner_contact_2: "", 
      address: "", border_color: "#000000", fill_color: "#000000", geojson: "" 
    });
  };
  ...
  const handleEdit = (station: FuelStation) => {
    ...
    setForm({
      name: station.name || "",
      company_id: station.company_id ? station.company_id.toString() : "",
      owner_name: station.owner_name || "",
      owner_contact_1: station.owner_contact_1 || "",
      owner_contact_2: station.owner_contact_2 || "",
      address: station.address || "",
      border_color: station.color || "#000000",
      fill_color: station.color || "#000000",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      owner_contact_1: form.owner_contact_1,
      owner_contact_2: form.owner_contact_2,
    ...
    try {
      if (editingStation) {
        await put(`/api/fuel-stations/${editingStation.id}`, payload);
        toast.success("Fuel station updated successfully.");
      } else {
        await post("/api/fuel-stations", payload);
  ...
  const handleDelete = async (station: FuelStation) => {
    try {
      await del(`/api/fuel-stations/${station.id}`);
      toast.success("Fuel station deleted."); loadData();
    } catch { toast.error("Failed to delete fuel station."); }
  };
  ...
  const mapRegions = stations.map(s => ({
    id: s.id, region_name: s.name, region_code: "", estimated_population: 0,
    region_type_title: "Fuel Station", parent_region_name: "", geojson: s.geojson, color: s.color,
  }));
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      <PageHeader
        title="Fuel Stations"
        description="Manage fuel station locations, company mappings, and ownership details."
        breadcrumbs={[{ label: "SWIFT", href: "/swift/shift" }, { label: "Fuel Stations" }]}
        actions={
          <Button onClick={formOpen ? closeForm : () => setFormOpen(true)} variant={formOpen ? "secondary" : "primary"}>
            {formOpen ? "✕ Close" : "+ Add Fuel Station"}
          </Button>
        }
      />
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pb-8">
        {formOpen && (
          <Card className="animate-fade-in relative z-20">
            <CardHeader>
              <CardTitle>{editingStation ? "✏️ Edit Fuel Station" : "⛽ Add Fuel Station"}</CardTitle>
              <CardDescription>Configure station details and draw or upload its geofence boundary.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Name *" placeholder="Eg. Geeta Bhawan" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                    <Select 
                      label="Company *" 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Owner Name" placeholder="Eg. Santosh Parik" value={form.owner_name} onChange={e => setForm({ ...form, owner_name: e.target.value })} />
                    <Input label="Owner Contact Number 1" placeholder="Eg. 9999999999" value={form.owner_contact_1} onChange={e => setForm({ ...form, owner_contact_1: e.target.value })} />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Owner Contact Number 2" placeholder="Eg. 9999999999" value={form.owner_contact_2} onChange={e => setForm({ ...form, owner_contact_2: e.target.value })} />
                  </div>
                  
                  <div className="flex flex-col">
                    <label className="text-xs font-semibold text-theme-text-dim uppercase tracking-wider mb-1.5">Address</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col">
                      <label className="text-[10px] text-theme-text-dim font-bold uppercase tracking-wider mb-1.5">GEOJSON/KML</label>
                ...
                {/* Map Side */}
                <div className="h-[500px] lg:h-[600px] border border-theme-border rounded-xl overflow-hidden relative shadow-inner bg-theme-surface">
                  <RegionMap geoJSON={form.geojson} color={form.fill_color} onChangeGeoJSON={val => setForm(prev => ({ ...prev, geojson: val }))} regions={mapRegions} isDrawing={isDrawing} setIsDrawing={setIsDrawing} editingRegionId={editingStation?.id} />
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="flex flex-col h-[600px]">
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <div><CardTitle>Fuel Stations Directory</CardTitle><CardDescription>Registered fuel station locations.</CardDescription></div>
            <Input placeholder="Filter stations..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-64" />
          </CardHeader>
          <CardContent className="p-0 flex-1 overflow-hidden">
            <div className="h-full overflow-y-auto custom-scrollbar">
              <Table
                headers={[<div key="s" className="text-center w-16">S. No.</div>, "Name", "Company", "Owner Name", "Monitor.OwnerNumber1", "Monitor.OwnerNumber2", "Address", <div key="a" className="text-right pr-4 w-24">Action</div>]}
                isLoading={loading}
                emptyState="No fuel stations found."
              >
                {filteredStations.map((station, idx) => (
                  <tr key={station.id} className="hover:bg-theme-base/40 transition-colors group">
                    ...
                    <td className="py-3 px-5 font-medium">{station.name}</td>
                    <td className="py-3 px-5 text-theme-text-dim text-xs">{station.company_name}</td>
                    <td className="py-3 px-5 text-theme-text-dim text-xs">{station.owner_name}</td>
                    <td className="py-3 px-5 text-theme-text-dim text-xs">{station.owner_contact_1}</td>
                    <td className="py-3 px-5 text-theme-text-dim text-xs">{station.owner_contact_2}</td>
                    <td className="py-3 px-5 text-theme-text-dim text-xs">{station.address}</td>
                    <td className="py-3 px-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <EditButton onClick={() => handleEdit(station)} />
                        <DeleteButton onDelete={() => handleDelete(station)} confirmMessage={`Delete ${station.name}?`} />
                      </div>
                    </td>
                  </tr>
                ))}
