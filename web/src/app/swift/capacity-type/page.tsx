...
export default function VehicleCapacityPage() {
  ...
  useEffect(() => {
    const cached = localStorage.getItem("swift:vehicle-capacities");
    if (cached) {
      try { setCapacities(JSON.parse(cached)); } catch (e) { setCapacities(DEFAULT_CAPACITIES); }
    } else {
      setCapacities(DEFAULT_CAPACITIES);
      localStorage.setItem("swift:vehicle-capacities", JSON.stringify(DEFAULT_CAPACITIES));
    }
  }, []);

  const saveCapacities = (updated: VehicleCapacity[]) => {
    setCapacities(updated);
    localStorage.setItem("swift:vehicle-capacities", JSON.stringify(updated));
  };
  ...
  return (
    <CrudDirectory
      ...
      breadcrumbs={[{ label: "SWIFT", href: "/swift/shift" }, { label: "Capacity Types" }]}
