...
export default function WorkshopPage() {
  const [stations, setStations] = useState<Workshop[]>([]);
  const [loading, setLoading] = useState(true);
  const [formOpen, setFormOpen] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [editingStation, setEditingStation] = useState<Workshop | null>(null);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await api<{ data: Workshop[] }>("/api/workshops");
  ...
  const closeForm = () => {
    ...
    setIsDrawing(false);
    setForm({ name: "", code: "", address: "", border_color: "#000000", fill_color: "#000000", geojson: "" });
  };

  const handleEdit = (station: Workshop) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.code.trim() || !form.address.trim()) {
      toast.error("Please fill Name, Code, and Address.");
      return;
    }
    ...
    try {
      if (editingStation) {
        await put(`/api/workshops/${editingStation.id}`, payload);
        toast.success("Workshop updated successfully.");
      } else {
        await post("/api/workshops", payload);
  ...
  const handleDelete = async (station: Workshop) => {
    try {
      await del(`/api/workshops/${station.id}`);
      toast.success("Workshop deleted."); loadData();
    } catch { toast.error("Failed to delete Workshop."); }
  };
  ...
  const mapRegions = stations.map(s => ({
    id: s.id, region_name: s.name, region_code: s.code, estimated_population: 0,
    region_type_title: "Workshop", parent_region_name: "", geojson: s.geojson, color: s.color,
  }));

  const filteredStations = stations.filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.address.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      ...
      <PageHeader
        title="Workshops"
        description="Manage waste Workshop locations and boundaries."
        breadcrumbs={[{ label: "SWIFT", href: "/swift/shift" }, { label: "Workshops" }]}
        actions={
          <Button onClick={formOpen ? closeForm : () => setFormOpen(true)} variant={formOpen ? "secondary" : "primary"}>
            {formOpen ? "✕ Close" : "+ Add Workshop"}
          </Button>
        }
      />
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pb-8">
        {formOpen && (
          <Card className="animate-fade-in relative z-20">
            <CardHeader>
              <CardTitle>{editingStation ? "✏️ Edit Workshop" : "🏢 Add Workshop"}</CardTitle>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input label="Code" placeholder="Eg. ws1" required value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} />
                  </div>
        ...
        <Card className="flex flex-col h-[600px]">
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <div><CardTitle>Workshops Directory</CardTitle><CardDescription>Registered Workshop locations.</CardDescription></div>
          ...
          <CardContent className="p-0 flex-1 overflow-hidden">
            <div className="h-full overflow-y-auto custom-scrollbar">
              <Table
                headers={[<div key="s" className="text-center w-16">S. No.</div>, "Name", "Code", "Address", <div key="a" className="text-right pr-4 w-24">Action</div>]}
                isLoading={loading}
                emptyState="No Workshops found."
              >
                {filteredStations.map((station, idx) => (
                  <tr key={station.id} className="hover:bg-theme-base/40 transition-colors group">
                    ...
                    <td className="py-3 px-5 font-medium">{station.code}</td>
                    <td className="py-3 px-5 text-theme-text-dim max-w-[200px] truncate" title={station.address}>{station.address}</td>
