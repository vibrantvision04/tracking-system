  ...
  const handleOpenForm = async (emp?: Employee) => {
    ...
    if (emp) {
      ...
      try {
        ...
        if (userRes.success) {
          const userEmail = `${emp.employee_id}@swift.com`;
      ...
      if (isEditing && formData.id) {
        ...
        if (res.success) {
          if (formData.login_password) {
            ...
            const userEmail = `${formData.employee_id}@swift.com`;
            try {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: "SWIFT", href: "/swift/shift" }, { label: "Employees" }]}
        actions={
        ...
        <Card>
          ...
          <CardContent className="p-0">
            ...
            <div className="p-4 border-t border-theme-border bg-theme-surface text-xs font-semibold text-theme-text-dim flex items-center justify-between">
              ...
              <span className="text-[10px] text-theme-text-dim uppercase tracking-widest font-mono">SWIFT EMPLOYEES</span>
            </div>
          </CardContent>
        </Card>
      </div>
