...
export default function VehicleMakePage() {
  ...
  useEffect(() => {
    const cached = localStorage.getItem("iswm:vehicle-makes");
    if (cached) {
      try { setMakes(JSON.parse(cached)); } catch { setMakes(DEFAULT_MAKES); }
    } else {
      setMakes(DEFAULT_MAKES);
      localStorage.setItem("iswm:vehicle-makes", JSON.stringify(DEFAULT_MAKES));
    }
  }, []);

  const saveMakes = (updated: VehicleMake[]) => {
    setMakes(updated);
    localStorage.setItem("iswm:vehicle-makes", JSON.stringify(updated));
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden select-none font-sans space-y-6 p-6 lg:p-8">
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: "ISWM", href: "/iswm/shift" }, { label: "Vehicle Makes" }]}
        actions={
