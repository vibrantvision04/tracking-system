...
export default function TransferStationPage() {
  ...
  const closeForm = () => {
    ...
    setIsDrawing(false);
    setForm({ name: "", address: "", border_color: "#000000", fill_color: "#000000", geojson: "" });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Failed to save transfer station.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (station: TransferStation) => {
    try {
      ...
      toast.success("Transfer station deleted."); loadData();
    } catch { toast.error("Failed to delete transfer station."); }
  };
  ...
  const mapRegions = stations.map(s => ({
    ...
    region_type_title: "Transfer Station", parent_region_name: "", geojson: s.geojson, color: s.color,
  }));
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      ...
      <PageHeader
        ...
        description="Manage waste transfer station locations and boundaries."
        breadcrumbs={[{ label: "ISWM", href: "/iswm/shift" }, { label: "Transfer Stations" }]}
        actions={
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pb-8">
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <Input label="Name" placeholder="Eg. IT Park" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
        ...
        <Card className="flex flex-col h-[600px]">
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <div><CardTitle>Transfer Stations Directory</CardTitle><CardDescription>Registered transfer station locations.</CardDescription></div>
          ...
          <CardContent className="p-0 flex-1 overflow-hidden">
            <div className="h-full overflow-y-auto custom-scrollbar">
              <Table
                headers={[<div key="s" className="text-center w-16">S. No.</div>, "Name", "Address", <div key="a" className="text-right pr-4 w-24">Action</div>]}
                isLoading={loading}
              ...
              >
                {filteredStations.map((station, idx) => (
                  <tr key={station.id} className="hover:bg-theme-base/40 transition-colors group">
                    ...
                    <td className="py-3 px-5 text-theme-text-dim">{station.address}</td>
