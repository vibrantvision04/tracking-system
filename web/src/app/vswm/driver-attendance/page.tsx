...
export default function DriverAttendancePage() {
  ...
  const fetchAttendance = async () => {
    try {
      setLoading(true);
      let path = "/api/attendance?role=driver";
      if (dateFilter) {
      ...
      toast.error("Failed to load driver attendance records");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRecords = records.filter((rec) => {
    ...
    return name.includes(q) || id.includes(q) || (rec.vehicle_no && rec.vehicle_no.toLowerCase().includes(q));
  });
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      <PageHeader
        ...
        description="Historical and current logs of driver punch-ins, including selfie verifications, helpers, and active vehicles."
        breadcrumbs={[{ label: "Attendance", href: "/vswm/employee" }, { label: "Driver Attendance" }]}
        actions={
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <div>
              ...
              <CardDescription>Review and audit all recorded driver punch events.</CardDescription>
            </div>
          ...
          <CardContent className="p-0">
              ...
              {filteredRecords.map((rec, idx) => {
                ...
                return (
                  <tr key={rec.id} className="hover:bg-theme-base/40 transition-colors group text-theme-text-dim text-xs">
                    ...
                    <td className="py-3 px-5 font-bold text-blue-400">{rec.vehicle_no || "—"}</td>
                    <td className="py-3 px-5 font-mono text-[11px]" title="Date/Time">{rec.punch_in_at}</td>
                    ...
                    <td className="py-3 px-5">
                      {rec.helper_present ? (
                        <div>
                          <span className="font-semibold text-theme-text">{rec.helper_name || "Yes (Declared)"}</span>
                          <span className="block text-[9px] text-emerald-400 font-bold uppercase tracking-widest mt-0.5">Helper Present</span>
                        </div>
                      ) : (
                        <span className="text-theme-text-dim/60">No Helper</span>
                      )}
                    </td>
                    <td className="py-3 px-5 truncate max-w-[120px]">{rec.ward_name || "—"}</td>
                    <td className="py-3 px-5">
                      {rec.gps_lat ? (
                        <a
                    ...
                    <td className="py-3 px-5">
                      {rec.is_valid ? (
                        ...
                        <span className="flex items-center gap-1.5 text-rose-400 font-semibold" title={rec.marked_by_name ? `Flagged (Manual Override by ${rec.marked_by_name})` : "Flagged Audit Fail"}>
            ...
            <div className="p-4 border-t border-theme-border bg-theme-surface text-xs font-semibold text-theme-text-dim flex items-center justify-between">
              ...
              <span className="text-[10px] text-theme-text-dim uppercase tracking-widest font-mono">VSWM ATTENDANCE</span>
            </div>
          </CardContent>
        </Card>
      </div>
