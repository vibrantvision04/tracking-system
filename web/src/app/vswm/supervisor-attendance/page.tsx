...
export default function SupervisorAttendancePage() {
  ...
  const fetchAttendance = async () => {
    try {
      setLoading(true);
      let path = "/api/attendance?role=supervisor";
      if (dateFilter) {
      ...
      toast.error("Failed to load supervisor attendance records");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden font-sans space-y-6 p-6 lg:p-8">
      <PageHeader
        ...
        description="Historical and current logs of Supervisor/CSI punch-ins, including location audits and selfie captures."
        breadcrumbs={[{ label: "Attendance", href: "/vswm/employee" }, { label: "Supervisor Attendance" }]}
        actions={
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <div>
              ...
              <CardDescription>Review and audit all recorded supervisor punch events.</CardDescription>
            </div>
              ...
              {filteredRecords.map((rec, idx) => {
                ...
                return (
                  <tr key={rec.id} className="hover:bg-theme-base/40 transition-colors group text-theme-text-dim text-xs">
                    ...
                    <td className="py-3 px-5 truncate max-w-[150px]">{rec.ward_name || "—"}</td>
