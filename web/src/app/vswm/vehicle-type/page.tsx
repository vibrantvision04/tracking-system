...
export default function VehicleTypePage() {
  ...
  useEffect(() => {
    if (!loaded) loadAll();
    const cached = localStorage.getItem("vswm:vehicle-types-meta");
    if (cached) {
  ...
  const saveMeta = (updated: Record<string, VehicleTypeMeta>) => {
    setMetaMap(updated);
    localStorage.setItem("vswm:vehicle-types-meta", JSON.stringify(updated));
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-theme-base text-theme-text overflow-hidden select-none font-sans space-y-6 p-6 lg:p-8">
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: "VSWM", href: "/vswm/shift" }, { label: "Vehicle Types" }]}
        actions={
