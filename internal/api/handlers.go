...
func (h *Handler) GetReports(w http.ResponseWriter, r *http.Request) {
	...
	// We only regenerate if the requested range includes today, or if forceRegen is true AND the request includes today.
	// Past days are NEVER synchronously regenerated by the Load button to preserve historical data.
	if hasToday {
		...
		for _, vehicle := range targetVehicles {
			...
			go func(v *repository.Vehicle, z, w string) {
				...
				// Always unfinalize today if forcing, so live updates overwrite DB correctly
				if forceRegen {
					_ = h.rService.UnfinalizeForDate(ctx, todayTime, v.ID)
				}
