...
func getDailyReportMutex(vehicleID int, dateStr string) *sync.Mutex {
	key := fmt.Sprintf("%d-%s", vehicleID, dateStr)
	val, _ := dailyReportLocks.LoadOrStore(key, &sync.Mutex{})
...
func (h *Handler) GetReports(w http.ResponseWriter, r *http.Request) {
	...
	useTimeStr := r.URL.Query().Get("use_time")
	...
	existingReports, _, err := h.rService.GetReports(r.Context(), vehicleID, from, to, 999999, 0, zoneID, wardID)
	...
	// 2. Fetch list of target vehicles and apply zone/ward filtering in-memory
	...
	if len(tasksToRun) > 0 {
		...
		for _, t := range tasksToRun {
			...
			go func(v *repository.Vehicle, targetDt time.Time) {
				...
				// Acquire lock for this vehicle and date to prevent concurrent execution races
				dateStr := targetDt.Format("2006-01-02")
				mu := getDailyReportMutex(v.ID, dateStr)
				mu.Lock()
				defer mu.Unlock()

				// Re-check if the report has already been generated by another concurrent thread
				if !forceRegen {
					existing, _, err := h.rService.GetReports(ctx, v.ID, targetDt, targetDt, 1, 0, 0, 0)
	...
	reports, total, err := h.rService.GetReports(r.Context(), vehicleID, from, to, limit, offset, zoneID, wardID)
	if err != nil {
		sendJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
