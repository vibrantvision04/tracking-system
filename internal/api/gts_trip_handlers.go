...
// TransferStationInfo and RouteCheckpointInfo are defined in gts_trip_helper.go
...
func (h *Handler) GetGTSTripReport(w http.ResponseWriter, r *http.Request) {
	...
	for idx, task := range tasks {
		...
		go func(idx int, task VehicleTask) {
			...
			// 4. Chronological State-Machine Validation Engine (using shared helper)
			tripCount, rejectedCount, rejectionReasons := CalculateValidatedGTSTrips(
				gpsData,
				wardPolygonPoints,
				checkpoints,
				transferStations,
