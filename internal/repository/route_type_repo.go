...
func (r *RouteRepository) GetAllRouteTypes(ctx context.Context) ([]RouteType, error) {
	query := `SELECT id, name, is_active, TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') FROM route_types_vswm ORDER BY id ASC`
...
func (r *RouteRepository) CreateRouteType(ctx context.Context, rt *RouteType) error {
	query := `INSERT INTO route_types_vswm (name, is_active) VALUES ($1, true) RETURNING id, is_active, TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS')`
...
func (r *RouteRepository) UpdateRouteType(ctx context.Context, id int, name string) error {
	_, err := r.db.Exec(ctx, `UPDATE route_types_vswm SET name = $1 WHERE id = $2`, name, id)
	return err
}
...
func (r *RouteRepository) DeleteRouteType(ctx context.Context, id int) error {
	...
	_, err := r.db.Exec(ctx, `DELETE FROM route_types_vswm WHERE id = $1`, id)
	return err
}
